        ...
        getSatisfiedAlarms: function(event, callback) {
            ...
            eventsRepository.getByTimeIntervalAndMeasureType(userId, "All", new Date(queryDateTime), dateTime, function (err, result) {
                if (err) {
                    ...
                    var lastTemperatureEvent = _.find(sortedResult, function (x) {
                        return x.measurementType === "temperature"
                    });
                    var lastBloodOxygenEvent = _.find(sortedResult, function (x) {
                        return x.measurementType === "bloodOxygen"
                    });
                    var lastHeartRateEvent = _.find(sortedResult, function (x) {
                        return x.measurementType === "heartRate"
                    });
                    var lastBloodPressureEvent = _.find(sortedResult, function (x) {
                        return x.measurementType === "bloodPressure"
                    });
                    ...
                    userDetailsRepository.findOneByEmail(userId, function (ud_err, userResult) {
                        if (ud_err) {
                            ...
                            patientsGroupMemberRepository.getByPatientId(userId, function (err, groupMembers) {
                                if (groupMembers && groupMembers.length > 0) {
                                    var groupId = groupMembers[0].groupId;
                                    patientGroupAlarmTemplateRepository.getAll({groupId:groupId}, function (g_err, alarmsResult) {
                                        if (g_err) {
